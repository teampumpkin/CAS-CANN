...
export class ZohoCRMLeadService {
  ...
  constructor() {
    ...
    console.log(
      `[ZohoCRMLeadService] Initialized with API domain: ${this.apiDomain}`,
    );
  }
  ...
  private formatDescription(formData: ZohoLeadData): string {
    const sections = [
      ...
      formData.institutionPhone
        ? `Institution Phone: ${formData.institutionPhone}`
        : "",
      "",
      "=== Amyloidosis Information ===",
      `Amyloidosis Type: ${formData.amyloidosisType || "Not provided"}`,
      formData.otherAmyloidosisType
        ? `Other Type Details: ${formData.otherAmyloidosisType}`
        : "",
      "",
      "=== Professional Interests ===",
      `Areas of Interest: ${formData.areasOfInterest || "Not provided"}`,
      formData.otherInterest
        ? `Other Interests: ${formData.otherInterest}`
        : "",
      `Presenting Interest: ${formData.presentingInterest || "Not specified"}`,
      formData.presentationTopic
        ? `Presentation Topic: ${formData.presentationTopic}`
        : "",
      "",
      "=== Consent Information ===",
  ...
  private async makeApiRequest(
    ...
    try {
      ...
      try {
        ...
        console.error(
          "[ZohoCRMLeadService] Failed to parse response:",
          responseText,
        );
      ...
      if (response.status === 429) {
        ...
        console.error(
          `[ZohoCRMLeadService] Rate limited. Retry after ${retryAfter} seconds`,
        );
        throw new Error(
      ...
      if (response.status === 401) {
        console.error(
          "[ZohoCRMLeadService] Authentication failed. Token may be invalid.",
        );
        throw new Error(
      ...
      if (!response.ok) {
        console.error(
          `[ZohoCRMLeadService] API error (${response.status}):`,
          responseData,
        );
