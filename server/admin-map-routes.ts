...
export function registerAdminMapRoutes(app: Express): void {
   ...
   */
  // Path is /api/admin/console/map/candidates on this branch: the legacy
  // AdminPortal already owns GET /api/admin/map/candidates (member-session auth).
  app.get("/api/admin/console/map/candidates", requireAdmin, async (_req: Request, res: Response) => {
    try {
      ...
      const published = await storage.getAdminMapClinics();
  ...
  app.post("/api/admin/map/publish", requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const saved = await storage.upsertAdminMapClinic({
  ...
  app.delete("/api/admin/map/published/:zohoRecordId", requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      await ensureMapClinicsTable();
      const removed = await storage.deleteAdminMapClinicByZohoId(zohoRecordId);
      if (!removed) {
  ...
  app.get("/api/map/clinics", async (_req: Request, res: Response) => {
    try {
      await ensureMapClinicsTable();
      const clinics = await storage.getAdminMapClinics();
      res.json({
