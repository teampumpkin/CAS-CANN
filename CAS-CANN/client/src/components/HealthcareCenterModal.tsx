    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="relative bg-gradient-to-r from-[#00AFE6] to-[#00DD89] p-6 text-white">
              <button
                onClick={onClose}
              ...
              <div className="flex items-start gap-3">
                <div className={`w-12 h-12 bg-gradient-to-r ${getTypeColor(center.type)} rounded-xl flex items-center justify-center shadow-lg`}>
                  <TypeIcon className="w-6 h-6 text-white" />
                </div>
                <div className="flex-1">
                  <h2 className="text-2xl font-bold mb-1 font-rosarivo">{center.name}</h2>
                  <div className="flex items-center gap-2 mb-1">
                    <MapPin className="w-4 h-4" />
                    <span className="text-base">{center.city}, {center.province}</span>
                  </div>
                  <div className="inline-flex items-center gap-2 bg-white/20 px-2 py-1 rounded-full text-xs">
            ...
            <div className="p-6 space-y-6">
              {/* Description */}
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3">{t('modal.aboutThisCenter')}</h3>
                <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-sm">{center.description}</p>
              </div>
              ...
              {/* Contact Information */}
              <div className="bg-gradient-to-r from-[#00AFE6]/10 to-[#00DD89]/10 dark:from-[#00AFE6]/5 dark:to-[#00DD89]/5 rounded-xl p-4">
                <h4 className="text-base font-semibold text-gray-900 dark:text-white mb-3">{t('modal.contactInformation')}</h4>
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <Phone className="w-4 h-4 text-[#00AFE6]" />
                      <span className="text-gray-700 dark:text-gray-300 text-sm">{center.contact.phone}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Mail className="w-4 h-4 text-[#00AFE6]" />
                      <span className="text-gray-700 dark:text-gray-300 text-sm">{center.contact.email}</span>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <motion.button
                  className="flex-1 bg-gradient-to-r from-[#00AFE6] to-[#00DD89] text-white px-5 py-2.5 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 text-sm"
                ...
                <motion.button
                  className="flex-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-5 py-2.5 rounded-xl font-semibold border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300 text-sm"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                ...
                {center.contact.website && (
                  <motion.button
                    className="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-all duration-300 text-sm"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
