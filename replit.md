...
- **Bulletproof Form Submission System**: Local-first architecture ensuring form submissions are saved to PostgreSQL immediately and then asynchronously synced to Zoho CRM. Two-phase retry strategy: fast retries (exponential backoff 10s-160s for first 5 attempts) then slow retries (5-minute intervals up to 50 total attempts). A background re-queue job runs every 5 minutes to rescue submissions stuck in failed state for 30+ minutes, with progressive backoff capping at 60-minute intervals. Submissions are effectively never permanently abandoned — only marked for manual review after 50 attempts. Includes automatic token refresh.
- **Centralized CRM Field Mapping** (`buildCentralizedZohoData` in `server/zoho-crm-service.ts`): Single source of truth for mapping form data to Zoho CRM fields. Enforces business rules: (1) CANN→CAS membership dependency (CANN_Member=true forces CAS_Member=true), (2) Record_Type classification ("Member" vs "Inquiry") for email campaign segmentation, (3) Lead_Source differentiation ("Website - Contact Inquiry" for non-members). Used by sync worker fallback, admin batch-update, and admin re-sync orphans. Smart mapper outputs are post-processed through this utility to ensure business rules and picklist-correct consent values always apply.
- **Admin Data Remediation** (`POST /api/admin/zoho/fix-membership-dependencies`): Scans existing synced records for CANN→CAS dependency violations and missing Record_Type fields. Supports dry-run mode. Fixes contradictory states and adds classification to historical records.
...
Event dates and past/upcoming detection on the Journal Club, Events, EventsOld, and CANN Resources pages use **EST (Eastern Standard Time)** via the `America/Toronto` timezone. This ensures:
